...
const textTokenizer = new Tokenizer('Hungarian');
...
const processHtmlContent = (htmlText) => {
    ...
    htmlDoc.body.childNodes.forEach((childNode, idx) => {
...
function wrapWordsAndSentences(node, htmlDoc, sentenceIdx = 0, wordIdx = 0) {
    if (node.nodeType === Node.TEXT_NODE) {
        ...
        textTokenizer.setEntry(textContent);
        const sentences = textTokenizer.getSentences();
